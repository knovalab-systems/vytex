...
func newTask(db *gorm.DB, opts ...gen.DOOption) task {
	...
	_task.Value = field.NewString(tableName, "value")
	_task.StepID = field.NewUint(tableName, "step_id")
	_task.Step = taskBelongsToStep{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Step", "models.Step"),
		Tasks: struct {
			field.RelationField
			Step struct {
				field.RelationField
			...
			RelationField: field.NewRelation("Step.Tasks", "models.Task"),
			Step: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Step.Tasks.Step", "models.Step"),
			},
		},
	}
...
func (t *task) updateTableName(table string) *task {
	...
	t.StepID = field.NewUint(table, "step_id")
...
func (a taskBelongsToStep) Where(conds ...field.Expr) *taskBelongsToStep {
	if len(conds) == 0 {
		return &a
	}
...
func (a taskBelongsToStep) WithContext(ctx context.Context) *taskBelongsToStep {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a taskBelongsToStep) Session(session *gorm.Session) *taskBelongsToStep {
	a.db = a.db.Session(session)
	return &a
}

func (a taskBelongsToStep) Model(m *models.Task) *taskBelongsToStepTx {
	return &taskBelongsToStepTx{a.db.Model(m).Association(a.Name())}
}

type taskBelongsToStepTx struct{ tx *gorm.Association }

func (a taskBelongsToStepTx) Find() (result *models.Step, err error) {
	return result, a.tx.Find(&result)
}

func (a taskBelongsToStepTx) Append(values ...*models.Step) (err error) {
...
func (a taskBelongsToStepTx) Replace(values ...*models.Step) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToStepTx) Delete(values ...*models.Step) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a taskBelongsToStepTx) Clear() error {
	return a.tx.Clear()
}

func (a taskBelongsToStepTx) Count() int64 {
	return a.tx.Count()
}
