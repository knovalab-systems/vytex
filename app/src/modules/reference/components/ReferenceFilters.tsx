...
function ReferenceFilters(props: { filters: ReferenceFilter; setFilters: Setter<ReferenceFilter> }) {
	const hasFilters = () => Object.keys(props.filters).length !== 0;
	const [open, setOpen] = createSignal(false);
	const [active, setActive] = createSignal(hasFilters());
	const [form, { Form, Field }] = createForm<ReferenceFilter>({ initialValues: props.filters });
	const { getColors, getColorsRecord } = useColors();

	const handleSubmit: SubmitHandler<ReferenceFilter> = data => {
		const filters: ReferenceFilter = {};
		for (const [k, v] of Object.entries(data)) {
			if (v && v.length > 0) {
				filters[k as keyof ReferenceFilter] = v as StateValues & number[];
				setActive(true);
			}
		}
